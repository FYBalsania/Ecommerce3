...
internal sealed class ProductGroupImageQueryRepository : ImageQueryRepository
{
    private readonly AppDbContext _dbContext;

    public ProductGroupImageQueryRepository(AppDbContext dbContext) : base(dbContext)
    {
        _dbContext = dbContext;
    }
    
    public override Type ImageType => typeof(ProductGroupImage);

    public override async Task<IReadOnlyList<ImageDTO>> GetByParentIdAsync(int parentId, CancellationToken cancellationToken)
    {
        return await _dbContext.ProductGroupImages.Where(x => x.ProductGroupId == parentId)
