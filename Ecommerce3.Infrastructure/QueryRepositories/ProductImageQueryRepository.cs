...
internal sealed class ProductImageQueryRepository : ImageQueryRepository
{
    private readonly AppDbContext _dbContext;

    public ProductImageQueryRepository(AppDbContext dbContext) : base(dbContext)
    {
        _dbContext = dbContext;
    }

    public override Type ImageType => typeof(ProductImage);

    public override async Task<IReadOnlyList<ImageDTO>> GetByParentIdAsync(int parentId, CancellationToken cancellationToken)
    {
        return await _dbContext.ProductImages.Where(x => x.ProductId == parentId)
