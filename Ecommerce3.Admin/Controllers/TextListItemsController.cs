...
public class TextListItemsController(
    ...
    ITextListItemService textListItemService)
    : Controller
{
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit([FromForm] EditTextListItemViewModel model,
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Delete([FromForm] DeleteTextListItemViewModel model,
        CancellationToken cancellationToken)
    {
        if (!ModelState.IsValid) return ValidationProblem(ModelState);
